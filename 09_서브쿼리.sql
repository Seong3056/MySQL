...
-- ROWNUM을 붙이고 범위를 지정해서 조회하려 하는데,
-- 범위 지정도 불가능하고, 지목할 수 없는 문제가 발생한다.
-- 이유: WHERE절부터 먼저 실행되고 나서 ROWNUM이 SLELECT 되기 때문이다.
-- 해결: ROWNUM까지 붙여넣고 다시 한번 자료를 SELECT 해서 범위를 지정해야한다.
...
/*
가장 안쪽 SELECT절에서 필요한 테이블 형식(인라인 뷰)를 생성
