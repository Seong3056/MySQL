...
- 서로 다른 테이블간에 설정된 관계가 결합하여 
  1개이상의 테이블에서 데이터를 조회하기 위해 사용한다.

 SELECT 컬럼리스트  FROM 조인 대상이 되는 테이블 ( 1개이상)
 WHERE 조인 조건 --> 오라클 조인문법
*/

-- employees 테이블으 ㅣ부서 id와 일치하는 departments 테이블의 부서 id를 찾아서 
...
테이블 이름을 작성해서 소속을 표현해 주는것이 바람직하다.
테이블 이름이 너무 길 때는 ALIAS를 작성하여 칭한다.
두 테이블 모두 가지고 있는 컬럼의 경우 반드시 명시해 줘야 한다.
*/

-- 3개의 테이블을 이요한 내부 조인 (INNER JOIN)
-- 내부 조인: 두 테이블 모두에서 일치하는 값을 가진 행만 반환한다.

SELECT
 e.first_name, e.last_name, 
 e.department_id, d.department_name, j.job_title
...
어느 한 테이블에 공통값이 없더라도 해당 row들이 조회결과에 
모두 포함되는 조인을 말한다.
*/
SELECT 
    e.first_name, e.last_name, e.hire_date,
    e.salary, job_id, e.department_id, d.department_name
FROM employees e, departments d, locations loc
WHERE e.department_id = d.department_id(+)
AND d.location_id = loc.location_id;
/*
employees 테이블에는 존재하고, departments 테이블에는 존재하지않아도
(+)가 붙징낳은 테이블을 기준으로 하여 departments테이블이 조인에
참여하라는 의미가 부여하기 위해 기호를 붙인다.
외부 조인을 사용 했더라도, 이후에 내부조인을 사용하면
내부조인을 우선적으로 인식한다.*/
/*
외부 조인 지냏ㅇ시 모든 조건에 (+)를 붙여야 외부조인이 유지된다.
일반 조건에도 (+)를 붙이지 않으면 외부조인이 풀리는 현상이 발생(데이터누락)
*/
SELECT
